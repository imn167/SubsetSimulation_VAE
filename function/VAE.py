...
class MoGPrior(tf.keras.Model):
  ...
  def sampling(self):
    ...
    for k in range(self.num_components):
      ...
      gaussians.append(Nk)
    mixture = tfd.Mixture(cat = w, components = gaussians)
    return mixture 
...
class AutoEncoder(tf.keras.Model):
  ...
  @tf.function
  def train_step(self, data): #tester et voir si avec une perte MSE ça marche 
...
class VAE(tf.keras.Model):
  ...
  def density_x(self, n_samples):#density for Vamprior prior
    # z = self.prior.sampling().getSample(n_samples) See for the MoGprior
    ...
    z = np.array(vamprior.sample(n_samples))
    idx = np.random.choice(np.arange(n_samples), size = n_samples) ###erreuuuur 
    zn = z[idx, :]
  ...
  def train_step(self, data):
    with tf.GradientTape() as tape :
      ...
      entropy =  tf.reduce_sum(-0.5 * (tf.math.log(2.0*np.pi) + 1 +  z_log_var), axis=1 ) #analytical expression of the entropy in a gaussian case 
